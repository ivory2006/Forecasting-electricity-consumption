...
train_dataset = read_csv('trainingB.csv',header=0,index_col=None)
train_values = train_dataset.values


test_dataset = read_csv('testingB.csv', header=0, index_col=None)
...
# fit network
history = model.fit(train_X, train_y, epochs=100, batch_size=64, verbose=2, shuffle=False)
...
df.to_csv("experimentB_LSTM_result.csv", index=False)
