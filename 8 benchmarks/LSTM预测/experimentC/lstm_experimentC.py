...
train_dataset = read_csv('trainingC.csv',header=0,index_col=None)
train_values = train_dataset.values

test_dataset = read_csv('testingC.csv', header=0, index_col=None)
...
# fit network
history = model.fit(train_X, train_y, epochs=2000, batch_size=32, verbose=2, shuffle=False)
...
df.to_csv("experimentC_LSTM_result.csv", index=False)
