...
def mul_info_predict(gru_test_file,le_test_file):
    ...
    mul_info_predict = np.array(gru_predict) * mul_info_weightC.gru_weight + np.array(le_predict) * mul_info_weightC.le_weight
    mape_list = []
    acc_list  = []
    mape = 0
...
if __name__ == "__main__":
    mul_info_predict("experimentC_GRU_result.csv","experimentC_LE_result.csv")