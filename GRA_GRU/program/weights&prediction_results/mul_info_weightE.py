...
def mul_info_weightE(gru_valid_file, le_valid_file):
    ...
    #gru与le准确率的商
    mul_accurary_division = []
    for i in range(len(gru_accurary)):
        if le_accurary[i] == 0:
            mul_accurary_division.append(1)
        else:
            mul_accurary_division.append(gru_accurary[i] / le_accurary[i])
    ...
    df.to_csv("experimentE_mutual_information.csv", index=False)
...
gru_weight, le_weight = mul_info_weightE("experimentE_GRU_validation.csv", "experimentE_LE_validation_result.csv")
