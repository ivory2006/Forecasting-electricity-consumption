...
def topn_optimal(n):
    ...
    for i in index:
        ...
        optimal = valid_start("valid7.csv", X)
        if optimal < max:
            Y = X
            max = optimal
...
if __name__ == "__main__":
    preprocessing("trainingB.csv", "train7.csv")
    preprocessing("testingB.csv", "test7.csv")
    train_start("train7.csv")
    test_start("test7.csv", 50)
