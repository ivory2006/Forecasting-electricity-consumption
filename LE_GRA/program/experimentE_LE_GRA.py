...
def preprocessing(before_filename, after_filename):
    ...
    common = [6 for i in range(power.shape[0])]
...
def topn_optimal(n):
    ...
    for i in index:
        ...
        optimal = valid_start("valid4.csv", X)
        if optimal < max:
            Y = X
            max = optimal
...
if __name__ == "__main__":
    preprocessing("trainingE.csv", "train4.csv")    
    preprocessing("testingE.csv", "test4.csv")
    train_start("train4.csv")
    test_start("test4.csv", 60)
