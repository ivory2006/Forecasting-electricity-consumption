...
def topn_optimal(n):
    ...
    for i in index:
        ...
        optimal = valid_start("valid8.csv", X)
        if optimal < max:
            Y = X
            max = optimal
...
if __name__ == "__main__":
    preprocessing("trainingA.csv", "train8.csv")
    preprocessing("testingA.csv", "test8.csv")
    train_start("train8.csv")
    test_start("test8.csv", 20)
